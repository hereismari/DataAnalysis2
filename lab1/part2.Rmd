...
Vamos ordenar os dados pela mediana, "plotar" o *Box Plot* de todos dados e considerar os 10 primeiros valores (menores medianas) e 10 últimos (maiores medianas) para nossa análise.
...
plot_10_maiores_medianas <- ggplot(dados_maiores_10_medianas_curso, aes(x = reorder(Cod_Curso,Media_Disciplina, FUN="median"), y = Media_Disciplina, fill = Nome_Curso_Campus)) + geom_boxplot() + xlab("Código do Curso") + ylab("Mediana do Curso") +
...
plot_10_menores_medianas <- ggplot(dados_menores_10_medianas_curso, aes(x = reorder(Cod_Curso, Media_Disciplina, FUN="median"), y = Media_Disciplina, fill = Nome_Curso_Campus)) + geom_boxplot() + xlab("Código do Curso") + ylab("Mediana do Curso") +
  labs(fill = "Nome do Curso : Campus") + theme(axis.text.x = element_text(angle = 90, hjust = 1)) +
...
Vamos utilizar o mesmo procedimento do *Box Plot*: visualizar os dados como um todo e depois selecionar os cursos com 10 maiores e menores médias.
...
ggplot() +
  ...
  geom_errorbar(data = dados_10_maiores_medias, aes(x = factor(reorder(Nome_Curso_Campus, Media_Curso)), ymin  = Media_Curso - Desvio_Curso, ymax = Media_Curso + Desvio_Curso), width = 0.4) + xlab("Nome do Curso : Campus") + ylab("Média do Curso") + coord_flip() + ggtitle("10 Maiores Médias Ordenadas")
...
ggplot() +
  ...
  geom_errorbar(data = dados_10_menores_medias, aes(x = factor(reorder(Nome_Curso_Campus, -Media_Curso)), ymin  = Media_Curso - Desvio_Curso, ymax = Media_Curso + Desvio_Curso), width = 0.4) + xlab("Nome do Curso : Campus") + ylab("Média do Curso") + coord_flip() + ggtitle("10 Menores Médias Ordenadas")
...
```
Em relação aos cursos de menores notas: ambas análises obtiveram resultados similares, concordando por exemplo que Física (Código: 11105110) apresenta as menores notas, porém apresetando desvio padrão bem alto, e o *Blox Plot* mostra uma grande dispersão dessas notas, porém com poucos (ou nenhum) *outliers*, devido ao baixo número de *outliers* a média deve representar bem os dados.
...
Os cursos de maiores notas apresentaram menor desvio padrão e mais *outliers*, devido ao grande número de *outliers* abaixo do resultado as médias devem estar um pouco a baixo da mediana, e as medianas devem ser mais confiáveis. Apesar disso os resultados são bem similares, havendo apenas alguns "desacordos" entre os *rankings* em relação a posição dos cursos.

Assim, podemos concluir a partir das análises que o curso que provavelmente tem as maiores notas é Direito - M do Campus 3. Além disso os cursos dos *rankings* são em sua maioria da área de saúde e humanas e algumas possibilidades para as notas serem tão altas são: baixo número de desistência, uma amostra indevida dos alunos (escolheu muitos alunos com boas notas), amostra de alunos muito pequena, ou simplesmente os alunos realmente apresentam um bom rendimento nesses cursos.
...
Nesta análise é considerado que a questão pede o período em que as disciplinas cursadas tiveram melhores notas e o período em que as disciplinas cursadas tiveram as menores notas. 
