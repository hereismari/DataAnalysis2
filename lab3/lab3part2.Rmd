...
Considerando apenas acurácia, o modelo global se saiu melhor com acurácia de 95.77% enquanto o modelo computação obteve acurácia de 95.24%. A diferença é bem pequena.
...
ggplot(data = df, aes(x=model, y=precision)) + geom_bar(stat="identity") + coord_cartesian(ylim=c(0.97, 0.98))
...
Todos os modelos obtiveram os mesmos resultados, resultados esses bastante satisfatórios por sinal, apenas a árvore de decisão do modelo computação obteve resultado superior nos treinos, porém inferior nos testes, o que pode ser um indicativo de overfitting. Na próxima seção iremos treinar modelos utilizando métodos para evitar overfitting.
